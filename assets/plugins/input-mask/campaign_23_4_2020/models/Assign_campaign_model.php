...
class Assign_campaign_model extends CI_Model {
    ...
    public function update_data($id, $data){
        foreach ($data as $key => $value) {
            $this->db->insert('v_assign_data', array('data_id'=>$value['data_id'], 'campaign_id'=>$id, 'assign_id'=>$value['assign_id'],'assign_date'=>date('Y-m-d H:i:s')));
        }
    ...
    public function unassign_agents($id){
        ...
        return $this->db->update($this->table.'_'.$id, array('assign_id'=>0, 'assign_date'=>null));
    }
    public function auto_assign($id){
        ...
        $total_data=$this->db->from($this->table.'_'.$id.' a')->count_all_results();

        $assign=$this->db->select('data_id, assign_id')->get($this->table.'_'.$id)->result();
        ...
        foreach ($assign as $key => $value) {
            if($value->assign_id != 0){
                $asgn[$value->assign_id][] = $value->data_id;
                $datasgn[$value->data_id][] = $value->assign_id;
            }
        }
        $flag = true;
        ...
        if($total_data>count($c)){
            $div = (int)($total_data/count($c));
            $mod = $total_data%count($c);
        }else{
            $div = 1;
            $mod = 0;
        }
        ...
        if(count($d)>0){
            ...
            for ($idx=0; $idx<count($c);$idx++) {
                $t = isset($asgn[$c[$idx]->assign_id]) ? ($div - count($asgn[$c[$idx]->assign_id])) : $div;
                if($t>0){
                    $t=$t-1;

                    for ($i=$t; $i >=0 ; $i--) { 
                        $d=$this->db->select('data_id')->where(array('assign_id'=> 0))->get($this->table.'_'.$id,1);
                        if($d->num_rows()>0){
                            $data_id = $d->row()->data_id;
                            $flag = $this->db->where('data_id',$data_id)->update($this->table.'_'.$id, array('assign_id'=>$c[$idx]->assign_id,'assign_date'=>date('Y-m-d'), 'retries'=>0));
                            $this->db->insert('v_assign_data', array('data_id'=>$data_id, 'campaign_id'=>$id, 'assign_id'=>$c[$idx]->assign_id,'assign_date'=>date('Y-m-d H:i:s')));
                        }
                    }
                }
                if($mod>0 && $idx == count($c)-1){
                    $d=$this->db->select('data_id')->where(array('assign_id'=> 0))->get($this->table.'_'.$id);
                    if($d->num_rows()>0){
                ...
                /*for ($i;$i<count($d);$i++) {
                    if(!$is_mod){
                        $data[]=array('data_id'=>$d[$i]->data_id, 'assign_id'=>$c[$idx]->assign_id, 'assign_date'=>date('Y-m-d'), 'retries'=>0);
                    }else{
                        $data[]=array('data_id'=>$d[$i]->data_id, 'assign_id'=>$c[($idx-($idx-$helper))]->assign_id, 'assign_date'=>date('Y-m-d'), 'retries'=>0);
                        $helper++;
                    }
                    $n = $div==1?0:1;
