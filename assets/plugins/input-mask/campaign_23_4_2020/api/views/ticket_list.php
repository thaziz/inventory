	      ...
	      <form id="search-form">
	        <div class="col-md-8 col-md-offset-2">
	            <div class="form-group" style="display: flex;margin-bottom:7px;">
	          	...
	          	<div class="form-group" style="display: flex;margin-bottom:7px;">
	              <label class="col-md-2">Caller Name</label>
	              <div class="col-md-6">
	            ...
	            <thead>
	              <tr>
	                ...
	                <th colspan="2" style="border-right-color: transparent;">
	                  ...
	                  <a href="<?=base_url('api/form_ticket/3ec8112b9e277cf4d24c85136fc9ee95?agent='.$user)?>" id="btn-new" class="btn btn-warning" target="_blank" style="display: none;">Create Ticket</a>
	      ...
	      data:{'order':sort, 'type':'a', 'filter':filter, 'ortype':ssort, 'start':start, 'adv_search':search, 'curr_assign':$('#curr_assign').val()}
	        ...
	        if(res.data.length>0){
		        $.when($.each(res.data, function(idx, val){
		          ...
		          tr += '<td><span><div><i class="fa fa-user"></i> '+val.cus_name+'</div><div class="small">City: '+val.city+'</div><div class="small">Country: '+val.country_origin+'</div></span></td>';
		          tr += '<td><div class="text-muted small"><span>Assign to</span> : '+(val.assign_user==null?val.assign_group:val.assign_user)+'</div><div class="text-muted small"><span>Status</span> : <label class="label label-'+(val.status=='OPEN'?'primary':(val.status=='CALLBACK'?'warning':'success'))+'">'+val.status+'</label></div><!--div class="text-muted small"><span>Need Callback</span> : '+(val.need_callback==1?'Yes':'No')+'</div--></td>';
		            tr += '<td>';
		            


		            tr+='<a href="<?=str_replace('covaid/', '', base_url()).'index.php?menu=ticket_data&action=detail&id='?>'+val.ticket_id+'" class="btn btn-default btn-xs"><i class="fa fa-list"></i> Detail</a>'
		        ...
		        $('#ticket_table tbody>tr').on('dblclick', function (e) {
		            e.stopPropagation();
		              ...
		              window.location = '<?=str_replace('covaid/', '', base_url()).'index.php?menu=ticket_data&action=detail&id='?>'+id;
		        });
		    }else{
		    	$('#btn-assign').prop('disabled', true);
		        $('#btn-closed').prop('disabled', true);
