...
class Ticket_api_model extends CI_Model {
   ...
   public function load_tickets($menu_role=array()){
        ...
        $this->db->select('e.username as assign_user, e.user_id, e.dept_id, e.department_name as assign_group, e.assign_time, e.is_read as c_read, a.open_by_name, a.*');
    ...
    private function query_where(){
        /*if(!$this->menu_role['v'] && !$this->menu_role['a'])
            $this->db->where('e.dept_id', $groupid);
*/
    ...
    private function search_advance($search){   
        ...
        if(!empty($search['subcat'])){
            $this->db->where('a.sub_category', $search['subcat']);
        }
        if(!empty($search['cat'])){
    ...
    private function filter($fil){
        ...
        $groupid = $this->session->userdata('dept_id');
        switch (strtolower($fil)) {
            ...
            case 'callback tickets':
                ...
                $this->db->or_where('e.dept_id', $groupid);
                $this->db->group_end();*/
                break;
            case 'closed tickets':
    ...
    public function get_detail($id){
        ...
        $this->db->join('v_caller c', 'c.cus_id=a.cus_id', 'left');
        $data['data'] = $this->db->get($this->table.' a', 1)->row();

        $assign = $this->db->select('a.dept_id, a.department_name, b.department_email')->join('v_department b', 'b.id=a.dept_id')->where(array('ticket_id'=>$data['data']->id, 'assign_id'=>$data['data']->curr_assign))
                    ->get('v_ticket_assignment a',1);
        if($assign->num_rows()>0){
            $data['assign_dept'] = $assign->row();
        }
    ...
    public function get_ticket_history($ticket_id){
        ...
        $this->db->select('a.history_id,a.user_id, a.username, a.activity, a.time');
    ...
    public function save_assign(){
        //if(!empty($_POST['assign_to'])){
            $assignment = $this->db->where('id',$_POST['assign_to'])->get('v_department',1)->row_array();
            $email_assign = $assignment['department_email'];
        /*}else{
        ...
        $_POST['user_id'] = $_POST['user_id'] === 'admin'?'123123':$_POST['user_id'];
        ...
        foreach (explode(',',$_POST['tick_ids']) as $key => $value) {
            ...
            $ticket = $this->db->select('ticket_id, subject, content, category, sub_category, cus_name, address cus_phone, district, city, province')->where('id', $value)->get('v_ticket', 1)->row_array();

            $ticket['assign_by']=isset($agent->name)?$agent->name:$agent->number;
            $ticket['assignee']=$assignment['department_name'];
            $ticket['assign_time']=date('d F Y H:i:s', strtotime($time));
            $ticket['due_time']=date('d F Y H:i:s', strtotime($time. ' +1 days'));
            ...
            if($this->db->insert('v_ticket_assignment',$assign)){
                ...
                $history = array(
                    ...
                    'cc'=>$cc,
                    'bcc'=>$bcc,
                    'activity'=>'Assign ticket to '.$assignment['department_name'].'.'.(!empty($note)?'<br><b>Note:</b><br>'.$_POST['assign_note']:'').(!empty($file)?'<br><b>Attachmet:</b><br>'.$file:''),
                );
                ...
                $email['note'] = $note;
                assign_email($email_assign,$ticket, false, $cc, $bcc, $attach);
            }
        }
        return true;
    }
    ...
    public function save_reply(){
        $agent = $this->db->where('number', ($_POST['user_id']=='admin'?'123123':$_POST['user_id']))->get('call_center.agent',1)->row();
        $time = date('Y-m-d H:i:s');

        $ticket = $this->db->select('ticket_id, subject, content, category, sub_category, curr_assign')->where('id', $_POST['tick_ids'])->get('v_ticket', 1)->row();
        $email = $_POST['reply_note'];

        $user_ext = $this->db->select('dept_id, department_name')->where(array('ticket_id'=>$_POST['tick_ids'], 'assign_id'=>$ticket->curr_assign))
                    ->get('v_ticket_assignment',1)->row();
        if(!empty($user_ext->department_name)){
            $assign = $this->db->select('department_email')->where(array('id'=>$user_ext->dept_id))->get('v_department',1)->row();
            $email_assign = $assign->department_email;
        }      
        ...
        if(!empty($_POST['to'])){
            $this->load->helper('mail');
            reply_email($_POST['to'],$ticket->ticket_id, $_POST['subject'], $_POST['cc'], $_POST['bcc'], $email.'<br><div>'.$thistory.'</div>', $attach);
        }
    ...
    public function save_comment(){
        ...
        $time = date('Y-m-d H:i:s');
        foreach (explode(',',$_POST['tick_ids']) as $key => $value) {
            $ticket = $this->db->select('ticket_id, main_category, subject, content, category, sub_category, curr_assign')->where('id', $value)->get('v_ticket', 1)->row();
            $user_ext = $this->db->select('user_ext')->where(array('ticket_id'=>$value, 'user_ext<>'=>($_POST['user_id']=='admin'?'123123':$_POST['user_id']), 'user_ext<>'=>0))
                    ->get('v_ticket_assignment')->result();
            $assignee = '';
            foreach ($user_ext as $key => $u) {
                if($u->user_ext!=($_POST['user_id']=='admin'?'123123':$_POST['user_id'])){
                    $assignee = $u->user_ext;
            ...
            $user_ext = $this->db->select('user_id')->where(array('ticket_id'=>$value, 'user_id<>'=>($_POST['user_id']=='admin'?'123123':$_POST['user_id']), 'user_id<>'=>0))->group_by('user_id')
                    ->get('v_ticket_history')->result();

            foreach ($user_ext as $key => $u) {
                if($u->user_id!=($_POST['user_id']=='admin'?'123123':$_POST['user_id']) && $u->user_id != $assignee){
                    write_notif($u->user_id, 'menu=ticket_data&action=detail&id='.$ticket->ticket_id, $ticket->ticket_id, '<strong>'.(isset($agent->name)?$agent->name:$agent->number).'</strong> comments on a ticket '.$ticket->ticket_id, $time, 'message');
                }
            }
    ...
    public function assignment_list(){
        $result = $this->db->select('id, department_name, department_email')->get('v_department')->result();
        $data = array();
        foreach ($result as $key => $value) {
            $data[] = array('id'=>$value->id, 'text'=>$value->department_name, 'email'=>$value->department_email);
        }
        return $data;
    }
    ...
    public function category(){
        $result = $this->db->select('id, category')->where(array('parent_id'=>0, 'enabled'=>1))->get('v_nticket_category')->result();
        $data = array();
        foreach ($result as $key => $value) {
    ...
    public function get_user_list(){
        return $this->db->select('a.number, a.name')
                ->where('estatus','A')->group_by('a.number')->get('call_center.agent a')->result();
    }
