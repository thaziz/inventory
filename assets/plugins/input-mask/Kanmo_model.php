...
* 
*/
class Kanmo_model extends CI_Model
{
	...
	public function check_caller($phone, $customer, $cus_fields, $ext_fields){
		$data = array(
			...
			'current_slab'=>$customer['current_slab'],
			'loyalty_points'=>$customer['loyalty_points'],
			'registered_store'=>$customer['registered_store']['name'],
			'fraud_status'=>$customer['fraud_details']['status'],
	...
	public function save_transaction($phone, $data){
		...
		if($query->num_rows() > 0){
			...
			$query = $this->db->where('cus_id', $cus_id)->get('v_transaction');
			if($query->num_rows()>0){
	...
	public function generate_ticket_id($ext){
		$ticket = date('ymd').'1'.substr($ext, -3);
	...
	public function save_ticket(){
		...
		if(!isset($_POST['cus_id'])){
			$cus = array('cus_fname'=>$_POST['cus_fname'],'cus_lname'=>$_POST['cus_lname'],'cus_phone'=>$_POST['cus_phone']);
		...
		$_POST['meta_category']=$this->db->where('id', $_POST['meta_category'])->get('meta_category',1)->row()->meta_category;
		$_POST['category']=$this->db->where('id', $_POST['category'])->get('v_kticket_category',1)->row()->category;
		if(is_numeric($_POST['sub_category'])){
			$_POST['sub_category']=$this->db->where('id', $_POST['sub_category'])->get('v_kticket_category',1)->row()->category;
		}
			...
			if($_POST['status']=='OPEN'){
				...
				if($this->db->insert('v_ticket_assignment',$assign)){
					...
					if($assignment['user_ext']!=0){
						write_notif($assignment['user_ext'], 'menu=kanmo_ticket&action=detail&id='.$_POST['ticket_id'], $_POST['ticket_id'], 'Ticket <strong>'.$_POST['ticket_id'].'</strong> is assigned to you', $time);
					}
					$this->load->helper('mail');
					assign_email(array('email'=>'support@kanmogroup.com', 'name'=>'Kanmo Retail'), $email_assign,$email, 'kanmo', false, $cc, $bcc);
				}
			}else{
				$history = array(
					'ticket_id'=>$ticket_id,
	...
	public function assignment_list(){
		$result = $this->db->select('id, name, email, user_ext')->get('v_assignment_list')->result();
		$data = array();
		foreach ($result as $key => $value) {
			$data[] = array('id'=>$value->id, 'text'=>$value->name, 'email'=>$value->email, 'ext'=>$value->user_ext);
		}
		return $data;
	}
	public function brand(){
		$result = $this->db->select('id, brand_name')->where('type','k')->or_where('type','b')->get('v_brand')->result();
		$data = array();
		foreach ($result as $key => $value) {
	...
	public function category($id){
		$result = $this->db->select('id, category')->where(array('meta_id'=>$id,'parent_id'=>0))->where('enabled',1)->get('v_kticket_category')->result();
		$data = array();
		foreach ($result as $key => $value) {
	...
	public function meta_category($id){
		$result = $this->db->select('id, meta_category')->where(array('calltype_id'=>$id))->get('meta_category')->result();
		$data = array();
		foreach ($result as $key => $value) {
    ...
    public function subcategory_info($subcat){
    	...
    	->join('v_kticket_category cat', 'cat.id=sub.parent_id')
    	->join('meta_category meta', 'cat.meta_id=meta.id')
    	->join('v_main_category type', 'type.id=meta.calltype_id')
    	->group_start()
    	//->like('cat.category',$subcat)
    	->or_like('sub.category',$subcat)
    	->group_end()
    	->where('sub.enabled',1)->get('v_kticket_category sub')->result_array();
    	return $result;
    }
/*    public function meta_category(){
		$result = $this->db->select('id,meta_category')->get('meta_category')->result();
		$data = array();
		foreach ($result as $key => $value) {
			$data[] = array('id'=>$value->id, 'text'=>$value->meta_category);
		}
