     ...
     public function get_chat($type,$agent,$phone){
        ...
        */
        ...
        foreach ($result as $key => $value) {
            ...
            $data[] = array('text'=>nl2br($value->wa_text), 'image'=>$value->wa_images, 'file'=>$value->wa_files, 'date'=>date('Y-m-d', strtotime($value->chat_time)), 'time'=>date('H:i', strtotime($value->chat_time)), 'status_chat'=>$status,'type'=>$value->is_reply, 'agent'=>($value->is_reply==0?$value->agent_name:''),'quote_text'=>nl2br($value->quoteText), 'quote_image'=>$value->quoteImage,'status'=>$value->status,'id_ack'=>$value->wa_id);
        }
        return $data;
    }
