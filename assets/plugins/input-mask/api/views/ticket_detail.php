	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="col-lg-11 col-md-11 col-sm-11">
				<div class="breacrumb">
					<span>
						<a href="<?=str_replace('kanmo/','index.php?menu='.$type.'_ticket',base_url())?>">Ticket</a>
					</span>
					<span>
					...
					<div class="row mb-3">
						...
						<?php if($editable==1):?>
							...
							<div class="btn-group btn-top" data-toggle="buttons">
								...
								<button class="btn btn-default" onclick="show_closed()" <?=($data->status!='CLOSED'?'':'disabled')?>>Closed</button>
							</div>
						<?php endif;?>
...
<?php  endif ?>
					...
					<div class="row mb-3" style="display: flex;">
						<div class="col-md-6">
							...
							<tbody><tr><td class="text-muted">First Name:</td><td><?=$data->cus_fname?></td></tr>
								<tr><td class="text-muted">Last Name:</td><td><?=$data->cus_lname?></td></tr>
								<tr><td class="text-muted">Phone:</td><td id="phone"><?=$data->phone?></td></tr>
								<tr><td class="text-muted">Email:</td><td><?=$data->email?></td></tr>
								<tr><td class="text-muted">Address:</td><td><?=$data->address_one_home_one?></td></tr>
								<tr>
						...
						<div class="col-md-6">
							<table class="borderless table-detail">
								<caption><strong>Information</strong></caption>
								<tbody><tr><td class="text-muted">Registered on:</td><td><?=$data->registered_on?></td></tr>
									<tr><td class="text-muted">Registered Store:</td><td><?=$data->registered_store?></td></tr>
									<tr><td class="text-muted">Subsidiary:</td><td><?=$data->subsidiary?></td></tr>
									<tr><td class="text-muted">Birthdate:</td><td><?=$data->birthdate?></td></tr>
									<tr><td class="text-muted">Anniversary:</td><td><?=$data->anniversary_date?></td></tr>
									<tr><td class="text-muted">Gender:</td><td><?=$data->gender?></td></tr>
								</tbody></table>
							<table class="borderless table-detail">
								<caption><strong>Transaction Summary</strong></caption>
								<tbody><tr><td class="text-muted">First Sale Date:</td><td><?=$data->first_sale_date?></td></tr>
									<tr><td class="text-muted">Last Sale Date:</td><td><?=$data->last_sale_date?></td></tr>
									<tr><td class="text-muted">Last Sale Amount:</td><td><?=$data->last_sale_amount?></td></tr>
									<tr><td class="text-muted">Last 12 Month Sale:</td><td><?=$data->last_twelve_mon_sale?></td></tr>
								</tbody></table>
						</div>
					</div>
					<div class="row mb-3">
				...
				<div class="col-md-8 pt-30">
					<div class="tabset">
						<ul class="nav nav-pills" role="tablist">
						  ...
						  <li <?=isset($reply)?'class="active"':''?>><a href="#email_customer" aria-controls="email_customer" role="tab" data-toggle="tab">Email Customer</a></li>
						  <li <?=isset($reply)?'class="active"':''?>><a href="#chat_customer" aria-controls="chat_customer" role="tab" data-toggle="tab">Chat Customer</a></li>
						</ul>
						...
						<div class="tab-content">
						  ...
						  <div role="tabpanel" class="tab-pane <?=isset($reply)?'active':''?>" id="email_customer">
						  	<div class="col-md-12 table-responsive" style="padding:0">
						  		<table class="table table-striped table-colored" id="tb-history" width="100%">
						  			<!--thead>
						  				<tr>
						  			...
						  			<tbody>
							  				<?php foreach ($email_customer as $key => $value):?>
							  				<tr>
							  					<td>
							  						<div><strong><?=$value->name?></strong></div>
							  						<div><p><?=date('F d, Y H:i', strtotime($value->time))?></p></div>
							  						<div><small><?=nl2br($value->content)?></small></div>
							  						<div><!--button type="button" title="reply comment" class="pull-right" onclick="reply_comment('<?=$value->history_id?>')"><i class="fa fa-comment-o"></i></button--></div>
							  					</td>
							  				</tr>
							  				<?php endforeach;?>
						  	...
						  	<div class="col-md-12 p-0">
						  		<button class="btn btn-default" onclick="reply_customer_mail()"><i class="fa fa-reply"></i> Reply Customer</button>
						  	</div>
						  </div>

						    <div role="tabpanel" class="tab-pane <?=isset($reply)?'active':''?>" id="chat_customer">
  ...
  <div class="inbox_people2">
	...
	<?php foreach ($chat_history as $key => $v):?>
			<?php $img = '';
$url_base="https://al-tele.dutamedia.com/kanmo/"; ?>
			<?php if($v->wa_images!=null) 
              if($v->wa_images!=null){
                $img = '<a href="'.$url_base.'assets/wa_images/'.$v->wa_images.'" data-fancybox="images"><img src="'.$url_base.'assets/wa_images/'.$v->wa_images.'" width="200px"></a><br>';
              }
             ?>
                      ...
                      echo '<div class="incoming_msg">'.
                                 ...
                                 '<div class="received_msg">'.
                                  '<div class="received_withd_msg">'.
                                    '<p>'.$img .' '.$v->wa_text.' </p>'.
                                    '<span class="time_date">'.date('H:i:s',strtotime($v->chat_time)).' | '.date('Y-m-d',strtotime($v->chat_time)).'</span>'.
                                  '</div>'.
                                '</div>'.
                             '</div>';
              }else{
      ...
      <div class="modal-header">
        ...
        <form role="form" id="assign-form">
          ...
          <div class="row">
            <div class="col-md-3 pl-20">
              ...
              <div class="form-group">
                ...
                <select class="form-control input-sm" name="assign_to" id="assign_to" style="height: 34px !important;width: 100% !important;">
                  <option value="" data-email="" selected> -- Select group or user -- </option>
                  <?php 
                  $listemail = array();;
            ...
            <div class="col-md-3" style="margin-top: 25px; width: 20%;">
              <div class="form-group">
            ...
            <div class="col-md-3" style="margin-top: 25px; width: 20%;">
              <div class="form-group">
                <input type="text" name="cc" id="cc" class="form-control" placeholder="cc">
                <span class="info"></span>
              </div>
            </div>
            <div class="col-md-3" style="margin-top: 25px; width: 20%;">
              <div class="form-group">
                <input type="text" name="bcc" id="bcc" class="form-control" placeholder="bcc">
                <span class="info"></span>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-11 px-20" style="width: 96%;">
              <div class="form-group">
                ...
                <div class="col-md-4 col-sm-4">
	                <input type="text" class="form-control select-tag2 input-xs tokenfield" multiple="multiple" name="to" value="<?=isset($email_assign)?$email_assign:''?>">
	            </div>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-11 px-20" style="width: 96%;">
              <div class="form-group">
                ...
                <div class="col-md-9 col-sm-9">
	                <input type="text" class="form-control select-tag2 input-xs" name="subject" value="Re: Ticket ID <?=$data->ticket_code?> need your response immediately.">
	            </div>
              </div>
            </div>
          </div>
...
function edit_ticket(id){
			...
			$.ajax({
			   ...
			   $.post('<?=base_url('api/ticket/get_source')?>/'+'<?php echo $type ?>')
		  		.done(function(res){
		  			$html='';
		  			$status='';
		  		...
		  		$.post('<?=base_url('api/ticket/get_meta_category')?>/'+'<?php echo $type ?>',{'id':v.main_id})
		  		.done(function(res){
		  			$html='';
		  			$status='';
		  			var data = JSON.parse(res);
		  		...
		  		$.post('<?=base_url('api/ticket/get_category')?>/'+'<?php echo $type ?>',{'id':v.meta_id})
		  		.done(function(res){
		  			$html='';
		  			$status='';
		  			var data = JSON.parse(res);
		  		...
		  		$.post('<?=base_url('api/ticket/get_sub_category')?>/'+'<?php echo $type ?>',{'id':v.cat_id})
		  		.done(function(res){
		  			$html='';
		  			$status='';
		  			var data = JSON.parse(res);
			...
			$('[name="main_category"]').change(function(){
		  		...
		  		$.post('<?=base_url('api/ticket/get_meta_category/')?>'+'<?php echo $type ?>',{'id':id})
		  		.done(function(res){
		  			var data = JSON.parse(res);
		  	...
		  	$('[name="meta_category"]').change(function(){
		  		...
		  		$('[name="category"]').html('<option value=""> --Category-- </option>');
		  		$('[name="sub_category"]').html('<option value=""> --Sub Category-- </option>');
		  		$.post('<?=base_url('api/ticket/get_category')?>/'+'<?php echo $type ?>',{'id':id})
		  		.done(function(res){
		  			var data = JSON.parse(res);
		  			$.each(data, function(i,v){
		  	...
		  	$('[name="category"]').change(function(){
		  		var id = $(this).val();
		  		$('[name="sub_category"]').html('<option value=""> --Sub Category-- </option>');
		  		$.post('<?=base_url('api/ticket/get_sub_category')?>/'+'<?php echo $type ?>',{'id':id})
		  		.done(function(res){
		  			var data = JSON.parse(res);
		  			$.each(data, function(i,v){
		  	...
		  	function save(){
		  		...
		  		$.ajax({
			      url: '<?=base_url('api/ticket/update_ticket')?>/'+'<?php echo $type ?>',
			      type: 'POST',
			      dataType: 'json',
