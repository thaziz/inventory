...
class Ticket extends MX_Controller {
	...
	public function ticket_list($type='nespresso',$auth=''){
			
		if($auth=='3ec8112b9e277cf4d24c85136fc9ee95'){

			if(isset($_POST['type'])){
				$_POST['type']=$type=='nespresso'?'NESPRESSO':'KANMO';
...
/*
	...
	*/
				...
				$param['maincat'] = $this->ticket_api_model->get_main_category();
				if($type=='kanmo'){
					$param['brand'] = $this->ticket_api_model->brand();
					$param['cat'] = $this->ticket_api_model->category('k');
					$param['subcat'] = $this->ticket_api_model->sub_category('n');
				}else{
					$param['cat'] = $this->ticket_api_model->category('n');
					$param['subcat'] = $this->ticket_api_model->sub_category('n');
					
				}
				$param['userlist'] = $this->ticket_api_model->get_user_list(strtoupper($type));
...
array("Volvo 3", "BMW 3", "Toyota 3"),*/
	...
	public function detail($type='kanmo',$auth){
		if($auth=='3ec8112b9e277cf4d24c85136fc9ee95'){
			...
			$data['maincat'] = $this->ticket_api_model->get_main_category();
				if($type=='kanmo'){
					$data['brand'] = $this->ticket_api_model->brand();
					$data['cat'] = $this->ticket_api_model->category('k');
					$data['subcat'] = $this->ticket_api_model->sub_category('n');
				}else{
					$data['cat'] = $this->ticket_api_model->category('n');
					$data['subcat'] = $this->ticket_api_model->sub_category('n');
