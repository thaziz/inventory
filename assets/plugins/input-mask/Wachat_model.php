...
class Wachat_model extends CI_Model {
    ...
    public function get_avail_agent($type){
        $data = $this->db->select('user_ext')->/*where('cluster', 'BOTH')->or_*/where('cluster', strtoupper($type))->get('kanmo.v_ext_cluster')->result();
        $ext = array();
        foreach ($data as $key => $value) {
            $ext[] = $value->user_ext;
        ...
        if ($q->num_rows()>0) {
            $result =  $q->result();
            foreach ($result as $key => $value) {
                if($this->check_count_handle_agent($value->id)<21) {
                    $data[] = $value;
                }
            }
            return $data;
    ...
    public function refresh_assign($number, $type) {
        if (!in_array($number, array('800','801','123123','123456'))) {
            $q = $this->db->select('id, name')->where(array('number'=> $number, 'estatus'=>'A'))->get('call_center.agent',1);
            $agent_id = 0;
            $agent_name = '';
            if($q->num_rows()>0){
                $agent_id = $q->row()->id;
            ...
            $total = 21 - $this->check_count_handle_agent($agent_id);
            //echo $total;
            if($total>0){
    ...
    function status_chat(){
         $id_agent=$this->get_id_agent($_POST['ext']);
