...
{/if}
    ...
    <div id="issabel-callcenter-wrap">
	    ...
	    <div id="issabel-callcenter-controles">
            ...
            <button id="btn_toggleacw" class="btn_acw {$CLASS_BOTON_ACW}" >{$BTN_ACW}</button>
            <button id="btn_togglehold" class="{$CLASS_HOLD}" >{$BTN_HOLD}</button>
...
{/if}
            ...
            <button id="btn_chat" class="issabel-callcenter-boton-activo" onclick="window.open( 'http://al-tele.dutamedia.com/kanmo/wa?issabel_user={$USER_EXT}&type=kanmo','_blank')">Whatsapp Chat</button>
	    </div> {* issabel-callcenter-controles *}
        <div id="aux-info">
            <ul id="aux-list">
                <!--li class="aux-item"><strong>Pray :</strong>00:15:00</li>
                <li class="aux-item"><strong>Pray :</strong>00:15:00</li>
                <li class="aux-item"><strong>Pray :</strong>00:15:00</li>
                <li class="aux-item"><strong>Pray :</strong>00:15:00</li-->
            </ul>
	    ...
	    <div id="issabel-callcenter-contenido">
			...
			<div id="issabel-callcenter-cejillas-contenido">
			   <div style="height: 35px;" class="scroll-left">
                    <p id="announce-text" style="margin:0;">Test</p>
                ...
                <div id="issabel-callcenter-llamada-paneles" style="position: absolute;left: -1000px;display: none">
                ...
                {foreach from=$CUSTOM_PANELS item=HTML_PANEL}
                <div id="tabs-{$HTML_PANEL.panelname}" style="display: block;">
                    {$HTML_PANEL.content}
                </div>
                {/foreach}
...
#issabel-callcenter-cejillas-contenido{
...
<script type="text/javascript">
  if (!window.EventSource) {
    var source = new EventSource("{$URL_AUX}");
    source.addEventListener('message', function(e) {
      var data = JSON.parse(e.data);
      var html = '';
      $.each(data.data, function(i,v){
        html += '<li class="aux-item"><strong>'+v.break+': </strong>'+v.val+'</li>';
      });
      $('#aux-list').html(html);
    }, false);

    var source = new EventSource("{$URL_ANNOUNCE}");
    source.addEventListener('message', function(e) {
      var data = e.data;
      $('#announce-text').html(data);
    }, false);
  } else {
