...
class Wa extends CI_Controller {
  ...
  public function lastchat($type){
   ...
   public function get_lastchat_chek($type){

      $chats = array();
      $chats = $this->wa_data_model->get_lastchat_chek($type,'816');
      echo json_encode($chats);    
  }
  ...
  public function f($type){

      $ticket = $this->wachat_model->find_ticket("6281285762081");
...
/*
  public function test($type) {
    ...
    public function chat_assign(){    
      ...
      $chat = $this->wachat_model->chat_assign();
        ...
        */
          $data['id']=$chat->id;
          $data['parent_number']=$chat->parent_number;
          $data['wa_number']=$chat->wa_number;
          $data['status']=$chat->status; 
          $data['chat_time']=date('Y-m-d H:i:s',strtotime($chat->chat_time));
          $data['is_reply']=$chat->is_reply; 
          $data['cust_id']=$chat->cust_id; 
          $data['agent_id']=$chat->agent_id; 
          $data['ticket_id']=$chat->ticket_id;
          $data['type']=strtoupper($chat->type);
          $data['agent_name']=$chat->agent_name;
          $data['wa_text']=$chat->wa_text;

          $a=$this->wa_data_model->get_customer_name($chat->wa_number);
          $data['wa_customer']=$a==' '?$chat->wa_number:$a;
          $data['time']=date('H:i:s', strtotime($chat->chat_time));
          $data['date']=date('Y-m-d', strtotime($chat->chat_time));
          $data['flag']=$chat->is_reply;
      

      $dashboard=$this->wa_data_model->dashboard($_POST["type"],$chat->agent_assign);              
      $history_chat=$this->wa_data_model->history_chat($_POST["type"],$chat->agent_id,$_POST['phone']);
      $agent_number= $_POST['agent_ass'];
      $oldagent= $_POST['number'];
              $dataPush = array(
                  'ext' => "wa$agent_number",
                  'data'    => $data,
                  'time'     => time(),
		...
		$data['agent_name']=$chat->agent_assign_name;
		//var_dump($chat->agent_assign_name);exit();
		$dataPush = array(
				'ext' => "wa-all",
				'data'    => $data,
				'time'     => time(),
  ...
  public function total_agent($type){
     ...
     public function agent_online($type)
    {
      ...
      function agent_ol($type){
        echo json_encode($this->wachat_model->get_online_agent_name($type));
    }

    function agent_ass($type){
        $a=$this->wachat_model->get_online_agent_name($type);

        $html='<select class="form-control" id="agent_ass">
                  <option value="">---Agent---</option>';
	        foreach ($a as $key => $v) {
	        	$html.="<option value=".$v->number.">".$v->number." - ".$v->name."</option>";
	        }
       $html.='</select>';
  ...
  public function get_ticket($type){
	...
	function storeToSpv(){
		var_dump($this->wachat_model->storeToSpv('kanmo'));exit();
	}
