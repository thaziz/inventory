...
public function find_data_id($phone, $sender=false){
    ...
    public function get_images_wa($cid){
     ...
     public function get_lastchat($type,$agent){
        ...
        */
        ...
        $sql="SELECT a.*, IFNULL(d.unread,0) as unread, IF(c.cus_fname IS NOT NULL, CONCAT(c.cus_fname,' ', c.cus_lname), a.wa_number) as fname,IF(a.agent_assign!=0,a.agent_assign_name,a.agent_name) nama_agent FROM `whatsapp_chat` a JOIN whatsapp_cust c ON (a.cust_id=c.cus_id) LEFT JOIN whatsapp_chat b ON (a.cust_id=b.cust_id AND a.wa_id<b.wa_id) LEFT JOIN (SELECT COUNT(wa_id) as unread, cust_id FROM whatsapp_chat WHERE status = 'unread' AND type = '$type' AND IF(`agent_assign` != '', `agent_assign`, `agent_id`) = $agent GROUP BY cust_id) d ON (a.cust_id=d.cust_id) WHERE IF(a.agent_assign != '', a.agent_assign, a.agent_id) = $agent AND a.status_chat<>'CLOSED' AND b.wa_id IS NULL AND a.type = '$type' GROUP BY a.wa_number order by a.chat_time desc";
        ...
        foreach ($result as $key => $value) {
            ...
            if($value->agent_assign!=0){
                ...
                $time_assign=$value->time_assign;
            }

            $my_chat[] = array('wa_id'=>$value->wa_id,'text'=>nl2br($value->wa_text), 'unread'=>$value->unread, 'name'=>$value->fname,'wa_number'=>$value->wa_number, 'date'=>date('Y-m-d', strtotime($value->chat_time)), 'time'=>date('H:i', strtotime($value->chat_time)),'name'=>$value->fname,'assign'=>$assign,'time_assign'=>$time_assign);
        }
     ...
     public function get_lastchat_chek($type,$agent){
    ...
    function agent($type,$number){
        $sql="SELECT a.number,a.name FROM call_center.agent as a join kanmo.v_ext_cluster as c on a.number=c.user_ext  where number!=$number and estatus='A' and cluster='$type'";
        $result=$this->db->query($sql)->result();
        return $result;
    }
