...
class Wachat_model extends CI_Model {
    ...
    public function get_online_agent_name($type){
        ...
        $this->db->select('ag.number,ag.name');
        $this->db->group_start();
    ...
    public function get_avail_agent_asli($type){
        ...
        if ($q->num_rows()>0) {
            $result =  $q->result();
            foreach ($result as $key => $value) {
                if($this->check_count_handle_agent($value->id)<50) {
                    $data[] = $value;
                }
            }
            return $data;
    ...
    public function refresh_assign($number, $type) {
        if (!in_array($number, array('800','801','123123','123456','823'))) {            
            ...
            $total = 50 - $this->check_count_handle_agent($agent_id);
            //echo $total;
            if($total>0){
...
//
    ...
    public function chat_assign(){
        $q = $this->db->select('id,name')
            ->where('number', $_POST['agent_ass'])
            ->where('estatus','A')
            ->get('call_center.agent',1);
        if($q->num_rows()>0){
            $agent_id=$q->row()->id;
            $agent_name=$q->row()->name;
        ...
        $chat=$this->db->select('*')
            ->where('wa_number', $_POST['phone'])
            ->join('(SELECT max(wa_id) wa_id from whatsapp_chat WHERE `wa_number` = "'.$_POST["phone"].'") as b','a.wa_id=b.wa_id')            
            ->get('whatsapp_chat as a',1);
        if($chat->num_rows()>0){
            $wa_id=$chat->row()->wa_id;

            $this->db->where('wa_id',$wa_id)
                ->update('whatsapp_chat', array('agent_assign_name'=>$agent_name,'time_assign'=>date('Y-m-d H:i:s'),'agent_assign'=>$agent_id));

                $datac=$chat->row();
                $datac->agent_assign_name=$agent_name;
    ...
    public function storeToSpv($type){
        $time=date('Y-m-d H:i:s');
        $data = $this->db->select('agent.id')->/*where('cluster', 'BOTH')->or_*/where('cluster', strtoupper($type))->where('(role="store")')->join('call_center.agent','number=user_ext')->get('kanmo.v_ext_cluster')->result();
        $id = array();
        foreach ($data as $key => $value) {
            $id[] = $value->id;
        }

        $q = $this->db->select('agent_id,wa_number,wa_id')->where(array('is_reply'=>0, 'status_chat<>'=>'CLOSED'))->where('TIMESTAMPDIFF(SECOND, chat_time, \''.$time.'\')<(60*10)')->where_in('agent_id', $id)
        ->order_by('chat_time','desc')->from('kanmo.whatsapp_chat',1);
        var_dump($q->get_compiled_select());
            exit();
        if($q->num_rows()>0){
