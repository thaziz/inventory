...
//        }
        ...
        {
            $link = $this->my_link_to_remote($this->base_url, $this->first_link, NULL);
        ...
        if  ($this->cur_page != 1)
        {
            ...
            $link = $this->my_link_to_remote($this->base_url, $this->prev_link, $pars);
        ...
        for ($loop = $start -1; $loop <= $end; $loop++)
        {
            ...
            if ($i >= 0)
            {
                ...
                else
                {
                    ...
                    $link = $this->my_link_to_remote($this->base_url, $loop, $pars);
        ...
        if ($this->cur_page < $num_pages)
        {
            ...
            $link = $this->my_link_to_remote($this->base_url, $this->next_link, $pars);
        ...
        if (($this->cur_page + $this->num_links) < $num_pages)
        {
            ...
            $pars = array($this->postVar=>$i);
            $link = $this->my_link_to_remote($this->base_url, $this->last_link, $pars);
    ...
    function my_link_to_remote($url, $text, $pars=array()){
        if($pars !== NULL):
        foreach($pars as $k=>$v):
            $par = $k.":".$v;    
        endforeach;        
        //$v = intval($v)/$this->per_page+1;
        
        //onclick='new Ajax.Updater('$div','$url',{method: 'post', parameters:{'.$par.'}, evalScripts:true}); return false;'
       
        $html = "<a href=\"\" data-page=\"$v\">$text</a>";
        else:
        $html = "<a href=\"\">$text</a>";
        endif;
    return $html;
