...
<div class="content-wrapper">
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li class="active">Agent Group</li>
    </ol>
  </section>
        ...
        <div class="box">
          <div class="box-header">
            ...
            <div class="action pull-right">
              ...
              <?php endif;?>
              <?php if($rules['c']):?>
              <a href="<?=base_url('panel/admin/agent_group/insert');?>" class="btn btn-success btn-sm btn-circle"><i class="fa fa-plus"></i> Insert</a>
              <?php endif;?>
            </div>
          </div>
          <div class="box-body">
...
<script type="text/javascript">
  function remove(id){
    if (confirm( "Are you sure you want to delete the selected agent group?" )) {
      $.ajax({
          url : '<?php echo base_url("panel/admin/agent_group/delete"); ?>',
          type: "POST",
          data : {'group_id':[id]},
          dataType: 'json',
  ...
  $(document).ready(function(){
    $('#group_table').dataTable({
      ...
      "ajax": {
              "url": "<?php echo base_url('panel/admin/agent_group'); ?>",
              "type": "POST"
          },
      "aaSorting": [[ 1, "desc" ]],
      "searching": true,
      ...
      { "sClass": "center", "aTargets": [ 4 ],
        "mRender": function(data, type, full) {
          var html = '<a href=<?=base_url('panel/admin/agent_group/detail');?>/' + full[0] 
              + ' class="btn btn-default btn-xs btn-col icon-black"><i class="fa fa-search"></i> Detail' 
              + '</a>';
                <?php if($rules['e']):?>
          html += '<a href=<?=base_url('panel/admin/agent_group/edit');?>/' + full[0] 
