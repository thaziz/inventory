					...
					<form class="form-horizontal" method="post" id="admin_form">
				        <div class="box-body">
					        <input type="hidden" name="adm_id"  value="<?=$data->adm_id?>">
				          	...
				          	<input type="hidden" placeholder="NIK" name = "adm_nik_old" id="name" class="form-control" value="<?=$data->adm_nik?>">
				          	<div class="form-group">
					            <label class="col-sm-2 control-label" for="adm_name">NIK *</label>
					            <div class="col-sm-4">
					              <input type="text" placeholder="NIK" name = "adm_nik" id="name" class="form-control" value="<?=$data->adm_nik?>" autocomplete="off">
					              <span class="info"></span>
					            </div>
				          	</div>
					              		...
					              		if($divisi){
						              		foreach ($divisi as $value) {
						              			echo '<option value="'.$value->id.'" '
						              					.($value->id==$data->adm_bidang?'selected':'').'>'
						              					.$value->name.'</option>';
						              		}
						              	}
					              	?>
					               	...
					               	<?php
					              		if($jabatan){
						              		foreach ($jabatan as $value) {
						              			echo '<option value="'.$value->id.'" '
						              					.($value->id==$data->adm_jabatan?'selected':'').'>'
						              					.$value->name.'</option>';
						              		}
						              	}
					              	?>
