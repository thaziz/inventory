<?php
    class Dashboard_campaign_model extends CI_Model
    {
        ...
        public function total_data_success($id)
        {
            ...
            $this->db->join('(SELECT * FROM call_attemp_'.$id.' WHERE status = \'Complete\' AND call_status = \'Contacted\' GROUP BY data_id) x','x.data_id = '.$this->table.$id.'.data_id');
        ...
        public function get_agent_perform($id, $limit, $order){
            ...
            $query = $this->db->query("SELECT x.*, ROUND((x.total_contacted/x.total_completed)*100,2) as rate FROM (SELECT c.adm_name, SUM(if(a.status='Complete',1,0)) as total_completed, sum(IF(d.call_status='Contacted', 1, 0)) as total_contacted FROM `data_campaign_$id` `a` JOIN `v_assign_campaign` `b` ON `a`.`assign_id`=`b`.`assign_id` JOIN `v_admin` `c` ON `c`.`adm_id`=`b`.`adm_id` LEFT JOIN (SELECT * FROM call_attemp_$id WHERE call_status = 'Contacted' AND status = 'Complete' GROUP BY data_id) `d` ON `d`.`data_id`=`a`.`data_id` GROUP BY `c`.`adm_id`) x ORDER BY `total_completed` $order, `total_contacted` $order, `rate` $order LIMIT $limit");
        ...
        {
            $result = array('contacted'=>0, 'not_answer'=>0,
            ...
            //$this->db->where(array('call_status<>'=>'', 'call_status<>'=>NULL));
        ...
        public function line_chart_data($id)
        {
            ...
            if($query->num_rows()>0){
                ...
                $this->db->select('DATE_FORMAT(attemp_date,"'.$sql_format.'") AS call_date, SUM(IF(a.call_status="Contacted",1,0)) AS contacted, SUM(IF(a.call_status="Not Answer",1,0)) AS no_answer, SUM(IF(a.call_status="Not Active",1,0)) AS no_active, SUM(IF(a.call_status="Reject",1,0)) AS reject, SUM(IF(a.call_status="Wrong Number",1,0)) AS wrong_numb, SUM(IF(a.call_status="Busy",1,0)) AS busy, SUM(IF(a.call_status="Callback",1,0)) AS callback', false);
