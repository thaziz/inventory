...
class Campaign extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('report_summary_campaign_model', 'report_summary_campaign');
    ...
    public function campaign_edit($id)
    {
        ...
        if ($_POST && $auth) {
            ...
            $data['arr_outbond'] = array('preview' => 'Preview', 'predictive' => 'Predictive');
            //$data['ivr_template']    =   $this->campaign_model->ivr_template();
            //$this->userlog->add_log($this->session->userdata['name'], 'ACCESS INSERT CAMPAIGN MENU');
    ...
    public function report($id)
    {
        ...
        if ($_POST && $auth) {
            ...
            foreach ($list as $report) {
                ...
                $row[] = !empty($report->contact_status) ? $report->contact_status : '';
                $row[] = !empty($report->uncontact_status) ? $report->uncontact_status : '';
    ...
    {
        $url = str_replace('outbound/', '', base_url()) . 'api/recording/?key=jkHAK23kjhsd223klja677skajskkjsh&filename=' . $filename;
        $this->load->view('play', array('url' => $url));
    }

    public function board_widget($id)
    {
        header('Content-Type: text/event-stream');
        header('Cache-Control: no-cache');
        $total_close            =    0;
        $total                    =    $this->dashboard_campaign->total_data_campaign($id);
        $success                 =    $this->dashboard_campaign->total_data_success($id);
        $precentages            =    number_format((float) ($total > 0 ? (($success / $total) * 100) : 0), 2, '.', '');
        /*$remain 				= 	$this->dashboard_campaign->total_data_remain($id);*/
        $aht                    =    $this->dashboard_campaign->average_handling_time($id);
        $line_chart                =    $this->dashboard_campaign->line_chart_data($id);
        $best                    =    $this->dashboard_campaign->get_agent_perform($id, 5, 'DESC');
        $worst                    =    $this->dashboard_campaign->get_agent_perform($id, 5, 'ASC');
        $call_status            =    $this->dashboard_campaign->call_status_data($id);
        $closing_number            =    $this->dashboard_campaign->success_selling($id);
        $success_selling        =    $this->dashboard_campaign->success_selling($id);

        for ($i = 0; $i < count($closing_number); $i++) {
            if (!isset($closing_number[$i]->result_status_number)) {
                $total_close += intval(0);
            } else {
                $total_close += intval($closing_number[$i]->result_status_number);
            }
        }

        echo "data: {\n";
        echo "data: \"total_data\":\"$total\",\n";
        echo "data: \"success\":\"$success\",\n";
        echo "data: \"success_percentage\":\"$precentages\",\n";
        /*echo "data: \"remaining_call\":\"$remain\",\n";*/
        echo "data: \"aht\":\"$aht\",\n";
        echo "data: \"call_status\":" . json_encode($call_status['data']) . ",\n";
        echo "data: \"call_status_total\":\"" . $call_status['total'] . "\",\n";
        echo "data: \"line_chart\":" . json_encode($line_chart) . ",\n";
        echo "data: \"best_agent\":" . json_encode($best) . ",\n";
        echo "data: \"worst_agent\":" . json_encode($worst) . ",\n";
        echo "data: \"closing_number\":" . json_encode($total_close) . ",\n";
        echo "data: \"success_selling\":" . json_encode($success_selling) . "\n";
        echo "data: }\n\n";
    }
    ...
    public function summary_report($id)
    {
        ...
        if ($_POST && $auth) {
            $form = $this->campaign_model->get_form($id);
            $form = json_decode($form);
            $list = $this->report_summary_campaign->get_load_data($id);
            $data = array();
            $no = $_POST['start'];
            foreach ($list as $report_data) {
                $no++;
                $row = array();
                $row[] = $report_data->name;
                $row[] = $report_data->date;
                $row[] = $report_data->AUX;
                $data[] = $row;
            }
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->report_summary_campaign->count_all($id),
                "recordsFiltered" => $this->report_summary_campaign->count_filtered($id),
                "data" => $data,
            );
            echo json_encode($output);
        } else {
            ...
            $data['canchange'] = $this->menu['rule']['panel/campaign']['e'];
            $data['rules'] = $this->menu['rule']['panel/campaign'];
            $this->template->view('view_campaign_summary_report', $data);
        }
    } */
    function call_p(){
        $a=$this->dashboard_campaign->call_p();
        var_dump($a);exit();
    }


    function tes(){
        $data['d'] =$this->dashboard_campaign->tes();
       ...
       // var_dump($data);exit();
        ...
        if ($_POST && $auth) {
            ...
            $this->template->view('view_campaign_dashboard1', $data);
        }
    }
}
