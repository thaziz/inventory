...
class Campaign_model extends CI_Model {
    ...
    public function detail_id($id){
        $this->db->select('a.campaign_id, a.campaign_name, concat(DATE_FORMAT(a.start_date,"%d %M %Y")," - ",DATE_FORMAT(a.end_date,"%d %M %Y")) as date_range, concat(TIME_FORMAT(a.stime_perday, "%h:%i %p")," - ", TIME_FORMAT(a.etime_perday, "%h:%i %p")) as schedule_per_day, retries, b.adm_name as created_by, c.adm_name as supervisi, a.date_create as created_at, a.script, a.sms_script, a.form,a.sms_enabled, a.wa_enabled,a.status', false);
        $this->db->join($this->pref.'admin b', 'b.adm_id=a.creator_id');
    ...
    public function insert(){
        ...
        if($this->db->insert($this->table, $data)){
            ...
            $tbl_camp .= '('.implode(', ',$column).', `callback_date` datetime DEFAULT NULL,`ptp_date` datetime DEFAULT NULL, PRIMARY KEY (data_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;';
            ...
            $alt_camp = 'ALTER TABLE data_campaign_'.$id.' '.implode(', ', $index).', ADD KEY `call_status`(`call_status`);';

            //build sql for creating table call_attemp
            /*$tbl_attemp = "CREATE TABLE `call_attemp_$id` (`attemp_id` int(11) NOT NULL AUTO_INCREMENT,`data_id` int(11) NOT NULL,`call_id` VARCHAR(50) NOT NULL,`agent_id` int(11) NOT NULL COMMENT 'caller',`attemp_date` datetime NOT NULL,`api_status` varchar(50) NOT NULL COMMENT 'status from api',`call_status` varchar(50) NOT NULL COMMENT 'status of call',`status` varchar(50) NOT NULL COMMENT 'status of data',`duration` TIME NOT NULL,`reason` text NOT NULL,".implode(', ',$column_call).", `callback` datetime DEFAULT NULL,`recordingfile` varchar(255) NOT NULL COMMENT 'recordingfile from call', PRIMARY KEY(`attemp_id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";*/
            $tbl_attemp = "CREATE TABLE `call_attemp_$id` (
                          ...
                          `ptp_date` datetime DEFAULT NULL,
                          `recordingfile` varchar(255) NOT NULL COMMENT 'recordingfile from call',
                          PRIMARY KEY(`attemp_id`)
                        ) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4;";
            //add sql for altering table indexing
            $alt_attemp = "ALTER TABLE `call_attemp_$id` ADD KEY `data_id` (`data_id`),ADD KEY `call_id` (`call_id`),ADD KEY `api_status`(`api_status`), ADD KEY `call_status`(`call_status`), ADD KEY `status`(`status`), ADD KEY `agent_id` (`agent_id`);";
            $tbl_error = array();
            try{
                    ...
                    if($this->db->query($alt_camp)){
                        if($this->db->query($tbl_attemp)){
                            if(!$this->db->query($alt_attemp)){
                                ...
                                if ($_POST['outbound_type'] === 'predictive') {
                                    $this->db->query("INSERT INTO `call_center`.`campaign_external_url`(`urltemplate`, `description`, `active`, `opentype`) VALUES('outbound/console/external_url/$id?phone={__PHONE__}&callid={__CALL_ID__}&campid={__CAMPAIGN_ID__}&calltype={__CALL_TYPE__}&agentnum={__AGENT_NUMBER__}', 'External url for ".$_POST['campaign_name']." campaign', 1, 'iframe')");

                                    $url_id = $this->db->insert_id();

                                    $this->db->query("INSERT INTO `call_center`.`campaign`(`name`, `datetime_init`, `datetime_end`, `daytime_init`, `daytime_end`, `retries`, `trunk`, `context`, `queue`, `script`, `estatus`, `id_url`) VALUES('".$_POST['campaign_name']."', '".$sdate."', '".$edate."', '".date('H:i:s', strtotime($_POST['stime_perday']))."', '".date('H:i:s', strtotime($_POST['etime_perday']))."', '".$_POST['retries']."', 'tr-dutamedia', 'from-internal', 905, '".$_POST['script']."', 'A', $url_id)");
                                    $cc_id = $this->db->insert_id();

                                    $this->db->query("UPDATE `outbound`.`v_campaign` SET call_center_campaign = $cc_id WHERE campaign_id = $id");
                                }
