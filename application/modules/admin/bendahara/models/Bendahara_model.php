   ...
   /*  function get_request_order(){
     ...
     function get_po_all($id){
       $th=$this->session->userdata('tahun');
        ...
        $this->db->select('po_id,po_code,po_code_a,c.d_name as fro,e.d_name as too, po_date,k_name as po_type,po_note,po_status,adm_name,po_from,po_to,po_anggaran,a_name,a_code,oa_saldo,a_id', false);
        $this->db->from('v_purchase_order');
        $this->db->join($this->pref.'divisi c','v_purchase_order.po_from = c.d_id');
        $this->db->join($this->pref.'divisi e', 'v_purchase_order.po_to = e.d_id');        
        $this->db->join($this->pref.'admin d', 'v_purchase_order.po_created_by = d.adm_id');        
...
function print_pengeluaran($id){
 ...
 public function ttd($id){
    ...
    public function detail($id){
         ...
         $this->db->select('po_id,po_code,c.d_name as fro,e.d_name as too, po_date,k_name,po_note,po_status,adm_name,po_from,po_to,c.d_code,po_code_a,po_type,po_anggaran,oa_saldo,po_kode_anggaran,f.a_name,f.a_code,po_request_id,po_ttd_bendahara', false);
        $this->db->from('v_purchase_order');
        $this->db->join($this->pref.'divisi c','v_purchase_order.po_from = c.d_id');
        $this->db->join($this->pref.'divisi e', 'v_purchase_order.po_to = e.d_id');        
        $this->db->join($this->pref.'admin d', 'v_purchase_order.po_created_by = d.adm_id');        
