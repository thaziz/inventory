...
<script type="text/javascript">
  function remove(id){
    if (confirm( 'Apakah kamu yakin ingin menghapus data barang?'  )) {
      $.ajax({
          url : '<?php echo base_url("panel/master/delete"); ?>',
          type: "POST",
          data : {'adm_id':[id]},
          dataType: 'json',
    ...
    $('#item_table').dataTable({
      ...
      { "sClass": "center", "aTargets": [ 4 ],
        "mRender": function(data, type, full) {
          console.log(full)
            <?php if($rules['v']){ ?>
          return ''+/*'<a href=<?=base_url('panel/master/detail');?>/' + full[3]
              + ' class="btn btn-default btn-xs btn-col icon-black"><i class="fa fa-search"></i> Detail'*/
              <?php } ?>
              <?php if($rules['e']){?>
              /*+ '</a>'+*/
              '<a href=<?=base_url('panel/master/edit');?>/' + full[3]
