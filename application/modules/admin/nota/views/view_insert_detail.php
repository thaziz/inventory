  ...
  <form class="form-horizontal" method="post" id="admin_form">
    <div class="row">
        <div class="col-md-12">
          ...
          <div class="col-md-4 col-sm-6 col-xs-12">
            <div class="form-group form-group-sm" id="div_kategori">
              <input type="text" name="total" id="total" class="form-control currency" placeholder="Anggaran" readonly>        
            </div>
          </div> 
  ...
  <tbody>
    <?php foreach ($po['detail'] as $key => $v): ?>
      <tr>
        ...
        <td>
          <input onkeyup="subharga(<?=$key?>)" type="text" value="0" name="harga[]" placeholder="Harga" class="form-control currency harga-<?=$key?> " >
        </td>
        <td>
          <input type="text" value="0" name="subharga[]" placeholder="Harga" class="form-control currency subharga-<?=$key?>" readonly>
        </td>
  
        <td>
          <textarea class="form-control" name="merk[]"></textarea>
        </td>
...
<script type="text/javascript">
  $(document).ready(function(){
    $('form#admin_form').on('submit', function(e) {
      ...
      $.ajax({
           url : '<?php echo base_url("panel/nota/insert/"); ?>',
          type: "POST",
