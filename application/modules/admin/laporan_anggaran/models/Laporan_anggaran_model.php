...
class Laporan_anggaran_model extends CI_Model {

    private $pref = '';
	var $table = 'jurnal';
	var $column_order = array('j_id','a_name','a_code','j_nominal','j_year');
	var $column_search =array('j_id','a_name','a_code','j_nominal','j_year');
	var $order = array('j_id' => 'asc');
	...
	private function load_admin(){
        ...
        $this->db->select('*');
        $this->db->join('v_opening_account op','op.oa_id=b.j_a_code');
        ...
        if(isset($_POST["adv_search"]['call_date'])){
            $tgl=$_POST["adv_search"]['call_date'];
             ...
             $this->db->where(array('j_date>='=>$sdate, 'j_date<='=>$edate));
        }
		$i = 0;
