          ...
          <div class="box-header">
            ...
            <div class="action pull-right">
             ...
             <!--  <a id="delete-all" title="Delete selected data" class="btn btn-danger btn-sm btn-circle"><i class="fa fa-trash"></i></a> -->
              <?php }?>
              <?php if($rules['c']){?>
              <a href="<?=base_url('panel/anggaran/insert_new');?>" class="btn btn-success btn-sm btn-circle"><i class="fa fa-plus"></i> Insert</a>
              <?php }?>
            </div>
          </div>
          <div class="box-body">
...
//console.log(full[7]);
             if(full[7]=='Permintaan')
              return '<span class="label label-danger">'+full[7]+'</span>';           
            if(full[7]=='Telaahan')
              return '<span class="label label-warning">'+full[7]+'</span>';
             if(full[7]=='Anggaran')
              return '<span class="label label-info">'+full[7]+'</span>';
             if(full[7]=='Pinjaman')
              return '<span class="label label-primary">'+full[7]+'</span>';
            if(full[7]=='Pengembalian')
              return '<span class="label label-success">'+full[7]+'</span>';
            if(full[7]=='Nota')
              return '<span class="label label-default">'+full[7]+'</span>';
        },
        "bSortable": false
      },
      /*{ "sClass": "center", "aTargets": [ 5 ], "data":4 ,
      ...
      { "sClass": "center", "aTargets": [ 9 ],
        "mRender": function(data, type, full) {
            ...
            <?php if($rules['v']){ ?>
               ...
               tbn+='<a href=<?=base_url('panel/anggaran/detail');?>/' + full[8]
              + ' class="btn btn-default btn-xs btn-col icon-black"><i class="fa fa-search"></i> Detail </a>';
              <?php } ?>
              
             /* if(full[9]==null || full[9]==''){
              <?php if($rules['c']){?>
              tbn+='<a href=<?=base_url('panel/anggaran/insert');?>/' + full[8]
              + ' class="btn btn-success btn-xs btn-col"><i class="fa fa-plus"></i> Insert' + '</a>';
              <?php }?>
              }
              if(full[9]=='xxx' || full[9]=='xxx'){            
              <?php if($rules['e']){?>
              tbn+= '<a href=<?=base_url('panel/anggaran/edit');?>/' + full[8]
              + ' class="btn btn-info btn-xs btn-col "><i class="fa fa-pencil"></i> Anggaran'
              <?php }?>
