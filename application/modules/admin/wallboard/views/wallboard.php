...
$search_f = isset($this->session->userdata['asearch']['ivr_search'])?$this->session->userdata['asearch']['ivr_search']:'';
?>
        ...
        <div class="box">
          <div class="box-header">
                        ...
                        <form class="form-horizontal" method="post" id="search_form">
                          <div class="col-md-12">
                            <div class="form-group">
                              <label class="col-sm-3 control-label" for="campaign_name">IVR Name </label>
                              <div class="col-sm-9">
                                <input type="text" name ="adv_search[ivr_name]" id = "adv_search[ivr_name]" class="form-control input-sm">
                              </div>
                            </div>

                            <div class="form-group">
                              <label class="col-sm-3 control-label" for="date">IVR Template </label>
                              <div class="col-sm-9">
                                <input class = "col-sm-4" type="text" name="adv_search[ivr_template]" class="form-control input-sm">&nbsp;&nbsp;
                                <input class = "col-sm-4" type="text" name="adv_search[ivr_template]" class="form-control input-sm">
                              </div>
                            </div>
                ...
                <div class="row">
                  <div class="col-md-12" style="text-align: center">
                    <div id="import" class="collapse" style="margin:15px auto; width: 450px;">
                      <form id="importform" method="post" action="<?=base_url('panel/ivr/import')?>" enctype="multipart/form-data">
                        <div class="form-group">
            ...
            <div class="action pull-right">
              ...
              <?php }?>
              <?php if($rules['c']){?>
              <a href="<?=base_url('panel/ivr/add');?>" class="btn btn-success btn-sm btn-circle"><i class="fa fa-plus"></i> Create</a>
              <?php }?>
...
<script type="text/javascript">
  function remove(id){
    if (confirm( "Are you sure you want to delete the selected ivr?" )) {
      $.ajax({
          url : '<?php echo base_url("panel/ivr/delete"); ?>',
          type: "POST",
          data : {'id':[id]},
          dataType: 'json',
  ...
  $(document).ready(function(){
    ...
    function reload_table() {
      $('#campaign_table').dataTable({
        ...
        "ajax": {
                "url": "<?php echo base_url('panel/ivr'); ?>",
                "type": "POST",
                "data": data_builder()
            },
        "aaSorting": [[ 2, "desc" ]],
