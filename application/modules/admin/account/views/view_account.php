        ...
        <div class="box">
          <div class="box-header">
            ...
            <div class="action pull-right">
              ...
              <?php }?>
              <?php if($rules['c']){?>
              <a href="<?=base_url('panel/account/insert');?>" class="btn btn-success btn-sm btn-circle"><i class="fa fa-plus"></i> Insert</a>
              <?php }?>
            </div>
          </div>
          <div class="box-body">
...
<script type="text/javascript">
  function remove(id){
    if (confirm( "Apakah Kamu Ingin Menghapus Data account?" )) {
      $.ajax({
          url : '<?php echo base_url("panel/account/delete"); ?>',
          type: "POST",
          data : {'d_id':[id]},
          dataType: 'json',
  ...
  $(document).ready(function(){
    ...
    $('#admin_table').dataTable({
      ...
      "ajax": {
              "url": "<?php echo base_url('panel/account'); ?>",
              "type": "POST"
          },
      "aaSorting": [[ 1, "desc" ]],
      "searching": true,
      ...
      { "title":"<input type='checkbox' class='check-all'></input>","sClass": "center","aTargets":[0],
        "render": function(data, type, full){
          return '<input type="checkbox" class="check-item" value="'+full[2]+'">';
        },
        "bSortable": false
      },
      ...
      { "sClass": "center", "aTargets": [ 3 ],
        "mRender": function(data, type, full) {
          ...
          return ''
              <?php } ?>
              <?php if($rules['e']){?>
              + ''+'<a href=<?=base_url('panel/account/edit');?>/' + full[2]
              + ' class="btn btn-info btn-xs btn-col icon-green"><i class="fa fa-pencil"></i> Edit'
              <?php }?>
              <?php if($rules['d']){?>
              + '</a>'+'<a href="javascript:;" onclick="remove(\'' + full[2] + '\');" id="btn-delete" class="btn btn-danger btn-xs btn-col icon-black"><i class="fa fa-close"></i> ' + 'Delete'
              <?php }?>
              + '</a>';
        },
        "bSortable": false
